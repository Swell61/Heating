...
void HeatingSystem::monitorSystem() { // This function runs through the process required to monitor and manage the heating system
	...
	if (remoteOption == Function::NewClient) { // If a new client has just joined
		...
		pinMode(ETH_CTRL_PIN, OUTPUT);
		digitalWrite(ETH_CTRL_PIN, LOW);
		...
		pinMode(ETH_CTRL_PIN, OUTPUT);
		digitalWrite(ETH_CTRL_PIN, HIGH);
	}
	...
	pinMode(5, OUTPUT);
	digitalWrite(5, LOW);
	updateDisplay = processInput(touchOption);
	pinMode(5, OUTPUT);
	digitalWrite(5, HIGH);
...
Function HeatingSystem::getRemoteOption) {
	pinMode(ETH_CTRL_PIN, OUTPUT);
	digitalWrite(ETH_CTRL_PIN, LOW);
	Function remoteOption = remote.processRemoteInput(buffer, config); // Process any clients
	pinMode(ETH_CTRL_PIN, OUTPUT);
	digitalWrite(ETH_CTRL_PIN, HIGH);
	return remoteOption;
}

void HeatingSystem::sendRemoteUpdate(Screen dataType) {
	pinMode(ETH_CTRL_PIN, OUTPUT);
	digitalWrite(ETH_CTRL_PIN, LOW);
	switch (dataType) {
		case Screen::Main:
...
bool HeatingSystem::processInput(Function userInputFunc) {
	switch (userInputFunc) {
